...
use crate::ops;
use crate::{Vec3, Vec3A, Vec4, Vec4Swizzles};
...
impl HalfSpace {
    /// Constructs a `HalfSpace` from a 4D vector.
    ///
    /// First 3 components represent the bisecting plane's unit normal,
    /// and the last component is the signed distance along the normal
    /// from the plane to the origin. The constructor ensures the normal
    /// vector is normalized and the distance is appropriately scaled.
    #[inline]
