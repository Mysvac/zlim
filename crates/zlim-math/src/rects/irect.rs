...
mod tests {
    ...
    #[test]
    fn rect_conversions() {
        let r = IRect::new(0, -2, 5, 3);
        let f = r.as_rect();
        assert_eq!(f.min, crate::Vec2::new(0., -2.));
        assert_eq!(f.max, crate::Vec2::new(5., 3.));

        // as_urect: negative components wrap (as u32), so use non-negative input.
        let r = IRect::new(0, 1, 5, 3);
        let u = r.as_urect();
        assert_eq!(u.min, crate::UVec2::new(0, 1));
        assert_eq!(u.max, crate::UVec2::new(5, 3));
    }
}
