...
impl Display for EvenCoreError {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::NotEnoughSamples { samples } => f.write_fmt(format_args!(
                "Need at least two samples to create an EvenCore, but {samples} were provided"
            )),
            Self::UnboundedDomain => {
                f.write_str("Cannot create an EvenCore over an unbounded domain")
            }
        }
    }
}

impl core::error::Error for EvenCoreError {}
...
impl Display for UnevenCoreError {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::NotEnoughSamples { samples } => f.write_fmt(format_args!(
...
impl core::error::Error for UnevenCoreError {}
...
impl Display for ChunkedUnevenCoreError {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::ZeroWidth => f.write_str("Chunk width must be at least 1"),
            Self::NotEnoughSamples { samples } => f.write_fmt(format_args!(
                "Need at least two unique samples to create a ChunkedUnevenCore, but {samples} were provided"
            )),
            Self::MismatchedLengths { expected, actual } => f.write_fmt(format_args!(
                "Expected {expected} total values based on width, but {actual} were provided"
            )),
            Self::NonDivisibleLengths {
                values_len,
                times_len,
...
impl core::error::Error for ChunkedUnevenCoreError {}
