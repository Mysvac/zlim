...
///
/// In single-threaded mode, all configuration methods are no-ops — there are
/// no worker threads to configure. The builder exists to maintain API parity
/// with the multi-threaded mode so that user code does not need conditional
/// compilation.
///
/// # Examples
///
...
///
/// This is the fallback implementation used when multi-threading is unavailable
/// or disabled (e.g., WASM, `no_std`, or when the `single_thread` feature is
/// enabled). All tasks execute on the calling thread — no background worker
/// threads are spawned.
///
/// # Executors
///
...
impl TaskPool {
    ...
    ///
    /// In single-threaded mode, this does not spawn any threads.
    #[inline]
    #[must_use]
    pub fn new() -> Self {
        TaskPool(())
    ...
    ///
    /// The task is submitted to the `LocalExecutor` and will not run until
    /// the executor is ticked (via [`run_local`] or [`scope`]). Returns a
    /// [`Task`] handle that can be awaited for the result.
    ///
    /// # Deadlock Warning
    ///
    /// Do **not** block the current thread waiting for the returned
    /// [`Task`] handle (e.g., via [`block_on`]) — the spawned task cannot
    /// run until this thread yields to the executor, so blocking will
    /// deadlock. Use [`TaskPool::scope`] to collect results synchronously.
    ///
    ...
    ///
    /// The task is submitted to the thread-local `LocalExecutor`. Because
    /// this executor is per-thread, the task never leaves the current thread
    /// and therefore does not require `Send`.
    ///
    /// As with [`spawn`], the task will not run until the executor is ticked.
    ///
    /// # Deadlock Warning
    ///
    /// Same deadlock risk as [`spawn`]: do **not** block waiting for the
    /// returned [`Task`] handle. Use [`TaskPool::scope`] to collect results
    /// synchronously.
    ///
    /// [`spawn`]: Self::spawn
    /// [`TaskPool::scope`]: Self::scope
...
// Placeholder implementation to maintain code consistency.
...
impl<'sco, 'env, T: Send + 'env> Scope<'sco, 'env, T> {
    ...
    ///
    /// In single-threaded mode, this delegates to [`spawn_local`]. The task
    /// will execute on the current thread and its result will be collected
