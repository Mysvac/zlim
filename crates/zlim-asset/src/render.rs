//! Where a renderable asset is used: [`RenderAssetUsages`].
...
bitflags::bitflags! {
    ...
    /// asset data from the CPU (via the `Assets<A>` resource) once unloaded (without re-loading it).
    ///
    ...
    /// in sequence and unload one before loading the next. See this [discussion about memory management]
    /// for more details.
    ///
    /// [discussion about memory management]: https://github.com/WebAssembly/design/issues/1397
    #[repr(transparent)]
...
impl Default for RenderAssetUsages {
    /// Returns the default render asset usage flags: `MAIN_WORLD | RENDER_WORLD`
    ///
