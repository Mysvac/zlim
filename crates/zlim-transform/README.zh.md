...
| `TransformPropagateStrategy` | 检测策略：`Default` / `PropagateAll` / `PropagateAllOnce`。 |
...
**`PropagateAll` 策略**：跳过 1-3 步，直接收集所有"根"（无上级实体、或上级实体无
`Transform` 组件）。此时阶段二总是 O(N) 随机访问（全树传播）。适合高度动态的场景。

**`PropagateAllOnce`**：与 `PropagateAll` 相同，但只生效一帧，帧末自动回退为
`Default`。适合场景切换（那一帧全树都要更新，直接全量传播省掉检测开销）。
...
多线程模式中，除了阶段 1-1 的“处理 ReparentSignal 消息”，其他步骤均可进行。
