...
//! |---|---|
//! | [`reflect_deserialize`](TypeDB::reflect_deserialize) | `{"my_crate::A": {"x": 3, "y": 4}}` |
//! | [`deserialize`](TypeDB::deserialize()) | `{"x": 3, "y": 4}` |
//!
//! [`reflect_deserialize`](TypeDB::reflect_deserialize) reads the type path from the input, resolves it
//! to a `&TypeDB` via [`TypeDB::get_by_path`], then deserializes the
//! payload.  This is **self-describing** — the input carries its own type
//! information.  [`deserialize`](TypeDB::deserialize()) expects only the payload; the caller must
//! already hold a `&TypeDB` for the target type.
//!
//! # Type naming in serde calls
//!
//! Only the outermost [`reflect_deserialize`](TypeDB::reflect_deserialize) resolves types via
//! [`type_path`](crate::path::TypePath::type_path). All inner serde calls
//! (`deserialize_struct`, `deserialize_enum`, `deserialize_tuple_struct`, etc.)
...
//! serde conventions and is the counterpart to the serialization naming.
//!
