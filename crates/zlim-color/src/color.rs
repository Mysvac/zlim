...
use crate::{Okhsla, Okhsva, Okhwba, Oklaba, Oklcha, Saturation, Srgba, Xyza};
...
///
/// This is useful when you need to store a color in a data structure that can't
/// be generic over the color type.
///
...
/// [luminance](Luminance) and [hue](Hue) adjustment, and [diffing](EuclideanDistance).
///
/// These operations delegate to the concrete color space contained by [`Color`],
/// but will convert to [`Oklch`](Oklcha) for operations which aren't supported in
/// the current space. After performing the operation, if a conversion was required,
/// the result will be converted back into the original color space.
///
/// ```rust
/// # use zlim_color::{Hue, Color};
...
/// ```
///
/// [`Oklch`](Oklcha) has been chosen as the intermediary space in cases where conversion
/// is required due to its perceptual uniformity and broad support for Bevy's color operations.
///
...
impl From<Okhsla> for Color {
    fn from(value: Okhsla) -> Self {
        Self::Okhsla(value)
    }
}

impl From<Okhsva> for Color {
    fn from(value: Okhsva) -> Self {
        Self::Okhsva(value)
    }
}

impl From<Okhwba> for Color {
