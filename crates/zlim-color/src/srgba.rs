...
use crate::{ColorToComponents, ColorToPacked};
...
impl StableInterpolate for Srgba {
    fn interpolate_stable(&self, other: &Self, t: f32) -> Self {
        let a: LinearRgba = (*self).into();
        let b: LinearRgba = (*other).into();
        LinearRgba::interpolate_stable(&a, &b, t).into()
    }
}
...
// Derived conversions through LinearRgba.
impl_from_via!(LinearRgba, Srgba, [Xyza]);
...
impl core::fmt::Display for HexColorError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::Parse(_) => f.write_str("Invalid hex string"),
            Self::Length => f.write_str("Unexpected length of hex string"),
            Self::Char(_) => f.write_str("Invalid hex char"),
        }
    }
}
...
impl From<core::num::ParseIntError> for HexColorError {
