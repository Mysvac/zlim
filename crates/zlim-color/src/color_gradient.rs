...
where
    T: Mix + Clone,
{
    /// Create a new [`ColorCurve`] from a collection of [mixable] types.
    ///
    /// The domain of this curve will always be `[0.0, len - 1]` where `len` is
    /// the amount of mixable objects in the collection.
    ///
