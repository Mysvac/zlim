...
impl Display for CubicBezierError {
...
impl<P: VectorSpace<Scalar = f32>> CubicGenerator<P> for CubicCardinalSpline<P> {
    ...
    fn to_curve(&self) -> Result<CubicCurve<P>, Self::Error> {
        ...
        let extended_control_points: Vec<&P> = once(&mirrored_first)
...
impl Display for CubicNurbsError {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::KnotsNumberMismatch { expected, provided } => f.write_fmt(format_args!(
                "Wrong number of knots: expected {expected}, provided {provided}"
            )),
            Self::DescendingKnots => f.write_str("Invalid knots: contains descending knot pair"),
            Self::ConstantKnots => f.write_str("Invalid knots: all knots are equal"),
            Self::WeightsNumberMismatch { expected, provided } => f.write_fmt(format_args!(
                "Incorrect number of weights: expected {expected}, provided {provided}"
            )),
            Self::NotEnoughControlPoints { provided } => f.write_fmt(format_args!(
...
impl Display for InsufficientDataError {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
...
#[cfg(test)]
mod tests {
    use crate::{CubicBSpline, CubicBezier, CubicGenerator, CubicNurbs};
    use crate::{CubicSegment, RationalCurve, RationalGenerator};
    use zlim_math::Vec2;
    use zlim_math::ops::{self, FloatPow};
    use zlim_math::vec2;
